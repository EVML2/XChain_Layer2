...
```
docker run -d --name=XChain_postgres -p 5432:5432 -e POSTGRES_PASSWORD=password postgres:14
docker exec -it XChain_postgres psql -U postgres -c 'CREATE DATABASE XChain_local'
docker exec -it XChain_postgres psql -U postgres -c 'CREATE DATABASE XChain_test'
```

If you restart your system running `docker start XChain_postgres` will bring the database back online.
...
- `LOCAL_DB_URL` - Database url used in `yarn start`. You most likely want to set this to `postgresql://postgres:password@localhost:5432/XChain_local`
- `TEST_DB_URL` - Database url used in `yarn test`. You most likely want to set this to `postgresql://postgres:password@localhost:5432/XChain_test`
...
```bash
LOCAL_DB_URL=postgresql://postgres:password@localhost:5432/XChain_local
TEST_DB_URL=postgresql://postgres:password@localhost:5432/XChain_test
DATABASE_URL=
...
```
https://api.XChain.com/status/prices?from=1687599307&to=1690191307
```
...
```
https://api.XChain.com/status/balances?chainId=1&from=1687599307&to=1690191307
```
...
```
https://api.XChain.com/status/reports/?chainId=1&type=CBV&from=1687599307&to=1690191307
```
...
```
https://api.XChain.com/status/discovery
```
